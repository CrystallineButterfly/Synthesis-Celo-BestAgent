...
MODULE = importlib.import_module('agents.celo_operator')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'celo_operator')
        self.assertEqual(spec.primary_contract_name, 'CeloPaymentController')
