...
contract CeloPaymentControllerTest is Test {
    CeloPaymentController private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_celo_bestagent");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new CeloPaymentController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        CeloPaymentController freshController = new CeloPaymentController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
